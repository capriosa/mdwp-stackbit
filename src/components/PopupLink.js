...
export default function PopupLink(props) {
    ...
    return (
        <section className="w-screen h-8 p-2 flex justify-center items-center bg-color-teal-300">
            <Link

                onClick={() => setPopup(!popup)}
                aria-label={altText}
                id={cssId}
                className={classNames('m-4', 'p-4', 'cursor-pointer', 'sb-component', 'sb-component-block', style === 'link' ? 'sb-component-link' : 'sb-component-button', cssClasses, {
            ...
            {popup &&
                <div className="flex w-2/3 h-1/2 justify-center p-10 mt-2"><span><a className="rounded-full  h-9 w-9 flex items-center justify-center  cursor-pointer bg-gray-500 m-2" onClick={() => setPopup(!popup)}>x</a></span><p className="bg-gray-400 p-4 flex-shrink lg:w-1/3">{props.body}</p></div>
